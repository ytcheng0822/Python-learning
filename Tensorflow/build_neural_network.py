...
def add_layer(inputs, in_size, out_size, activation_function=None):  # activation_function(激勵函數)
    Weights = tf.Variable(tf.random.normal([in_size, out_size]))  # normal distribution
...
y_data = np.square(x_data) - 0.5 + noise         # output data
...
train_step = tf.compat.v1.train.GradientDescentOptimizer(0.1).minimize(loss) # 優化器:訓練自己修正誤差 learning rate = 0.1
